...
obj/Technique/FakeMoon
	verb/Fake_Moon()
		...
		if(usr.doing)
			...
			return
		if(usr.fused||!usr.has_tail)return
		if(usr.race=="Saiyan"&&usr.rage_chromosome&&usr.form_5&&!usr.form_6&&usr.powerlevel_max>=3500000)
			if(!usr.Oozaru)
				usr.doing = 1
				usr.icon_state = "enrage"
				view(8) << "[usr] shoots a blast up in the sky..."
				usr.frozen = 1
				sleep(35)
				...
				view(8) << "<font color = #FFFF00>[usr] screams out loud as golden fur sprouts out of their body and they turns Golden Oozaru!"
		...
		if(!usr.Oozaru)
			...
			view(8) << "[usr] starts charging up a ball of energy..."
			usr.frozen = 1
			sleep(25)
			view(8) << "[usr] shoots the energy ball up at the sky..."
			usr.icon_state = ""
			usr.overlays-=icon('Techs.dmi',"fakemoono")
			...
			view(8) << "<font color = #996600>[usr] screams out loud as fur sprouts out of their body and they turns Oozaru!"
			usr.icon = 'Oozaru.dmi'
			usr.icon_state = ""
...
mob
	proc
		OozaruTrans()
			if(src.fused||!src.has_tail)return
			if(!src.form_1)
				if(!src.Oozaru)
					src.doing+=1
					src.icon_state = "enrage"
					view(8)<<"[src] starts to stare at the moon..."
					src.frozen+=1
					sleep(25)
					...
					view(8)<<"<font color = #996600>[src] screams out loud as fur sprouts out of their body and they turns Oozaru!"
					src.icon = 'Oozaru.dmi'
					src.icon_state = ""
...
mob/proc/Oozaru_Revert()
	...
	if(src.Oozaru)
		...
		if(src.race == "Saiyan" || src.race=="Half Saiyan" && src.has_tail)src.overlays += 'Tail.dmi'
		src.Power_Redefine()
		src.Skin_Apply()
		src.Hair_Apply()
		view() << "[src] begins to shrink down to their normal size."
		src.boxing = 0
		src.dualtrain = 0
		src.rest = 0
		src.doing = 0
		...
		if(!src.skin)
			src.skin="tan"
			src.Skin_Apply()
			src<<"<font color=red>Your skin as been set to tan. If this wasn't your character's skin, ask a GM for help."
		spawn(10)if(src)src.Hair_Apply()
		return
mob/proc/Forced_Oozaru_Revert()
	if(src.Oozaru)
		...
		src.Skin_Apply()
		src.Hair_Apply()
		oview() << "[src] begins to shrink down to their normal size."
		src.boxing = 0
		src.dualtrain = 0
		src.rest = 0
		src.doing=0
		...
		if(src.form_1)
			...
			src.Skin_Apply()
			src.Hair_Apply()
			oview() << "[src] has reverted from their transformed state."
			src.icon_state = ""
			src.boxing = 0
			src.dualtrain = 0
			src.rest = 0
