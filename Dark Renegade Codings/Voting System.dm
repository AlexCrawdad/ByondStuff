		...
		Mute
			...
			New()
				...
				spawn while(1)
					...
					if(time<=0)
						var/playerson=0
						for(var/mob/PC/M in world)if(M.client)playerson+=1
						if(yes>no)
...
world
	proc
		RemoveDelay(K)
			spawn(3000)if(VoteDelay.Find("[K]"))VoteDelay.Remove("[K]")
		...
		AutoVoteBoot(K)
			...
			for(var/mob/M in world)if(M.key==K)del(M)
			spawn(3000)if(VoteBoot.Find("[K]"))VoteBoot.Remove("[K]")
mob
	PC
		verb
			Vote_Mute()
		...
		VoteMute()
			...
			switch(alert("Do you want to vote for a mute on [B]?","Vote Mute","Yes","No"))
				if("No")
					...
					if(lentext(reason)>25)reason=copytext(reason,1,26)
