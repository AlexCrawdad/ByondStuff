...
turf
	Space
		...
		SpaceLimitL
			Enter(mob/M)
				if(istype(M,/mob)||istype(M,/obj))M.x=390
		SpaceLimitR
			Enter(mob/M)
				if(istype(M,/mob)||istype(M,/obj))M.x=10
		SpaceLimitU
			Enter(mob/M)
				if(istype(M,/mob)||istype(M,/obj))M.y=10
		SpaceLimitD
			Enter(mob/M)
				if(istype(M,/mob)||istype(M,/obj))M.y=390
obj
	Spaceship
		SpacePod
			name="Space Pod"
							...
							if(D<=0)
								for(var/mob/M in src)
									if(Z==1)
										...
										if(prob(33))src.dir=NORTH
										else
											if(prob(50))src.dir=SOUTH
											else src.dir=EAST
									if(Z==2)
									...
									src.density=1
									src.travel=1
									spawn(10)if(src)src.TravelSpace()
							sleep(1)
				TravelSpace()
					step(src,src.dir)
