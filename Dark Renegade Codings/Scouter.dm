				...
				if(!usr.scouter_eq)
					...
					if(usr.scouter)
						if(equiped)
							...
							usr<<"Error: </b>You may only have one of: <u>[src]</u> equiped."
							return
		verb
			Pick_Up()
				set src in oview(0)
			...
			Scouter_Channel()
				...
				if(src.equiped)
					...
					if(scouter_chan<0)scouter_chan=0
					usr.scouter_channel=scouter_chan
					return
				else usr<<"You must have the: <u>[src]</u> equiped before proceeding."
			Scouter(T as text)
				set category=null
				if(src.equiped)
					if(channelmute!="")
					...
					if(usr.oocmute)
						...
						return
					if(usr.say_delay>=3)
						world<<"<font color = #00B211>Dragon AI:</font> \white [usr] has been automatically muted for 30 seconds for \red spamming."
						usr.say_delay=0
						usr.oocmute=1
						spawn(300)if(usr)usr.oocmute=0
						return
					if(!T)return
					T=html_encode(T)
					if(lentext(T)>150)
					...
					sleep(10)
					usr.say_delay-=1
					for(var/mob/PC/M in world)if(M.scouter_eq&&M.scouter_channel==usr.scouter_channel)M<<"<font color=#000066>{<font color=lime><font face = Tahoma>Scouter([usr.scouter_channel])<font color=#000066>} \white [usr]: \green [T]"
				else
			...
			Scan(mob/M in oview(8))
				...
				if(equiped)
					if(M.powerlevel<=100)
						usr<<"Error: Unable to read [M] powerlevel!"
						return
					usr<<"---------------------"
					...
					usr<<"Ki Def: [num2text(M.kidefense,10000000)] / [num2text(M.kidefense_max,10000000)]"
					...
					usr<<"You must have: <u>[src]</u> equiped."
					return
