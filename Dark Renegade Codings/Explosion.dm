proc
	Explode(Effect/theeffect)
		if(theeffect.orig)new /ExplodeNode/CenterAll(theeffect,theeffect.orig.x,theeffect.orig.y,theeffect.basepower)
	ExplodeBM(Effect/theeffect)
		if(theeffect.orig)new /ExplodeNode/CenterBM(theeffect,theeffect.orig.x,theeffect.orig.y,theeffect.basepower)
ExplodeNode
	CenterBM
			...
			if(round(power) > 0)
				if(x >= 1 && x <= world.maxx && y >= 1 && y <= world.maxy)
					...
					switch(dir)
						...
						if(WEST)tx = -1
						if(EAST)tx = 1
					if(theeffect.speed)spawn(theeffect.speed)src.New(theeffect,x+tx,y+ty,power,dir)
