...
mob/var/custom_name2="Custom Attack2"
mob/var/custom_name3="Custom Attack3"
...
mob/var/learn_customattack2=0
mob/var/learn_customattack3=0
obj/Technique/CustomAttack
obj/Technique/CustomAttack2
obj/Technique/CustomAttack3
	verb
		CustomTechnique()
			set category="Techniques"
			set name="Custom Attack"
			...
			spawn(5)if(usr)
				...
				usr.doing=0
				usr.using=0
			spawn(15+usr.custom_delay)if(usr)usr.tech_cooldown=0
			var/obj/Tech/BeamH/A=new
			if(!usr.custom_homing)return A.TechAttackStraight(usr,"[usr.custom_name]",usr.ki*usr.custom_ki+usr.custom_damage*3,usr.custom_icon,"","",0)
			else
