...
obj/Technique/Kaioken
	verb/Kaioken()
		...
		if(usr.race=="Half Majin"||usr.race == "Bio-Android"||usr.race=="Namekian"||usr.race=="Majin"||usr.race=="Tuffle")kaioken_limit=100
		if(usr.race=="Dragon")kaioken_limit=150
		...
		if(kaioken_bonus>kaioken_limit)
			view(8)<<"<font color=red>[usr] collapses from the strain of Kaioken!"
			usr.kaioken=0
			usr.overlays-='Kaio Aura.dmi'
			usr.doing=0
			usr.sight|=BLIND
			...
			if(kaioken_bonus==kaioken_limit)
				...
				usr.frozen=0
				usr.doing=0
				usr.powerlevel+=(rand(300,600)+(usr.powerlevel_max*(kaioken_bonus/10)))
				usr.ki+=(rand(300,600)+(usr.ki_max*(kaioken_bonus/10)))
				usr.overlays+='Kaio Aura.dmi'
				usr.speed=usr.speed_max-1000
				if(usr.speed<1000)usr.speed=1000
				usr.kidefense+=(rand(300,600)+(usr.kidefense_max*(kaioken_bonus/10)))
				usr.strength+=(rand(300,600)+(usr.strength_max*(kaioken_bonus/10)))
				usr.defence+=(rand(300,600)+(usr.defence_max*(kaioken_bonus/10)))
				usr.Kaio_Strain()
			else
				if(kaioken_bonus>=1)
					usr.icon_state="enrage"
					...
					usr.speed=usr.speed_max-1000
					if(usr.speed<1000)usr.speed=1000
					usr.kidefense+=(rand(300,600)+(usr.kidefense_max*(kaioken_bonus/32)))
...
mob/proc/Kaio_Strain()
	...
	if(!kaioken)
		...
		if(stamina>=stamina_max)
			...
			doing=0
			sight&=~BLIND
		else spawn(40)if(src)Kaio_Strain()