	...
	verb/Talk()
		...
		if(usr.race!="Konat")
			if(usr.playing)
				...
				if(usr.stamina>0)
					usr.stamina=0
				spawn(3000)if(usr)usr.playing = 0
		else
			if(!usr.tapionquest1)
			...
			if(!usr.tapionquest2)
				...
				for us to be locked inside. But, now the time is right. A wicked man created an evil creature which feeds it's \
				neverending hunger on people's fear. That creature could not be stopped. So me and my brother, had to lock \
