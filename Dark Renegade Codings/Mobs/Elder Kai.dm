			...
			verb
				Talk()
					...
					if(src.doing)
						...
						return
					if(usr.potara_fused)
						usr<<"\white Elder Kai: \font Erm, it may looks like you already potara'd! I simply cannot do this anymore!"
						return
					if(usr.z_sword_quest)
						if(usr.karma == "Good" && usr.karma_rating >= 30)
							if(usr.race == "Half Saiyan"||usr.race == "Namekian" && usr.powerlevel_max >= 5000000)
								if(!usr.gone_mystic)
									src.doing = 1
									...
									sleep(10)
									switch(alert("Do you wish to train with the Elder Kai?(Training takes 15 minutes)","","Yes","No"))
										if("Yes")
											if(usr.doing)
												usr<<"You cannot do mystic training whilst doing something else!"
												return
											usr.training=1
											usr.doing=1
											usr.mystic = 1
											...
											usr.safe = 0
											usr.speed_max-=2000
											if(usr.speed_max<1000)usr.speed_max=1000
