mob
	NPC
		Yardrat_Barber
			...
			verb
				Talk()
					...
					usr.talking=1
					if(usr.race == "Saibaman"||usr.race=="Galactic Warrior"||usr.race=="Kai" || usr.race == "Changeling" || usr.race == "Tuffle" || usr.race=="Bio-Warrior")
						usr<<"\white Barber:\font I can only help people with hair."
						usr.talking=0
						return
					if(usr.race=="Namekian" || usr.race=="Bio-Android" || usr.race=="Majin" || usr.race=="Demon")
						usr<<"\white Barber:\font I can only help people with hair."
						usr.talking=0
						return
		...
		Yardrat_ClothingShopKeeper
			name = "{NPC}Yardrat Clothing Shop Keeper"
			icon_state="Yardrat"
			density = 1
			safe = 1
			New()
		...
		Yardrat_ArmorShopKeeper
			name = "{NPC}Yardrat Armor Shop Keeper"
			icon_state="Yardrat"
			density = 1
			safe = 1
			New()
		...
		Yardrat_Sensei
			...
			verb
				Talk()
					...
					usr.talking = 1
					if(!usr.learn_it)
						switch(alert("Hi [usr]. We Yardratians are specialists in teleporting. \
						...
						if("Yes")
							switch(input("Where do you wish to use your TPs?","TPs")in list("Powerlevel","Ki","Ki Defense","Strength","Defense","Stamina","Cancel"))
								if("Cancel")
									...
									return
								if("Exp")
									var/TP = input ("How many TPs do you wish to use? [usr.tp] TPs") as num
									var/pl = 20 * TP
									if(TP > usr.tp)
