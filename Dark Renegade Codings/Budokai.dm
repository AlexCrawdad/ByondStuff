...
mob
	...
	PC/verb
		Watch_Budokai()
			...
			set name="Tournament TV"
			if(!tourny_watch)
				ceye=client.eye
				client.perspective=EYE_PERSPECTIVE
				client.eye=locate(155,66,6)
				tourny_watch=1
				return
			else
				client.eye=ceye
					...
					if(!in_tourney)
						switch(alert("Do you want to Join the Budokai?","Budokai Registration","Yes","No"))
							if("No")return
								...
								loc=locate(rand(76,96),rand(69,74),7)
				...
				switch(input("What do you want to do?","Budokai")in list("Start Registration","End Registration","Start Next Match","Budokai Boot","Cancel Budokai","Budokai Announce","Give Prize","Cancel"))
					...
					if("Budokai Announce")
						...
						if(!t||t=="")return
							...
							for(var/mob/M in world)if(M.in_tourney)M<<"<font color=#CC0000><center>Budokai:<br><br>[t]<br>"
							usr<<"<font color=#CC0000><center>Budokai:<br><br>[t]<br>"
					if("Budokai Boot")
						...
						switch(alert("Boot [A]?","","Yes","No"))
							if("Yes")
								...
								if(reason)world<<"<font color=red>Budokai:<font color=white> [usr] \red Booted [A]([reason])"
								else world<<"<font color=red>Budokai:<font color=white> [usr] \red Booted [A]"
								if(!A)return
								A.in_tourney=0
					...
					if("Cancel Budokai")
						...
						world<<"<font color=red>Budokai: \white [usr] cancelled the Budokai!"
						return
					if("End Registration")
						tourney_register=0
						for(var/mob/M in world)M.verbs-=new/mob/Tourney/verb/Budokai_Registration
					...
					if("Start Registration")
						...
						if(!limit)return
						else tourney_limit=limit
						prize=input("Set the Zenni prize.")as num
						if(prize<0)prize=0
						if(prize>10000000)prize=10000000
						tourney_register=1
						tourney_on=1
						TourneyPeople=new/list()
	...
	proc
		Budokai_Check()
			if(tourney_on&&tourney_register)
				verbs += new/mob/Tourney/verb/Budokai_Registration
				return
			else
				verbs -= new/mob/Tourney/verb/Budokai_Registration
				return
proc
	BudokaiResign(mob/N)
		...
		for(var/mob/PC/M in world)
			if(M==N)continue
			if(M==budofighter1||M==budofighter2)M.loc=locate(rand(76,96),rand(69,74),7)
		budofighter1=0
		budofighter2=0
		N.in_tourney=0
		return
		...
		BudokaiOutside
			...
			Enter(var/mob/PC/M)
				...
				if(M==budofighter1||M==budofighter2)
					...
					return
				else M.loc=locate(rand(76,96),rand(69,74),7)