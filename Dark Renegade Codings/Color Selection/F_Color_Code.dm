		...
		ShowBackground(xval, yval, client/C)
			for(var/i in 1 to 6)
				for(var/j in 1 to 4)
					...
					O.icon = 'F_Select_Background.dmi'
					O.icon_state = "[i],[j]"
...
obj/FTCS_Object
	color_slider
		...
		var
			...
			Yval
			Xval
			f_color_selector/Owner
		New(loc,position, s_loc, f_color_selector/F)
			..()
		...
		MouseUp(location, control, params)
			...
			Owner.Update_Color(base_color, base_value+2*icon_x)
...
obj/FTCS_Object
	...
	ok_buttons
		...
		var
			f_color_selector/Owner
		New(loc,s_loc,letter, client/C, f_color_selector/F)
			..()
			Owner = F
			screen_loc = s_loc
	...
	cancel_button
		...
		var
			f_color_selector/Owner
		New(loc,s_loc, client/C, f_color_selector/F)
			..()
			Owner = F
			screen_loc = s_loc
	...
	help_button
		...
		Click()
			..()
			usr << {"
<b><center><font color = green>Help</b>
<font color = lime>To  choose a color, simply click on the red, green or blue selection
bars until you get the color you desire. Once you have the color you
desire, simply click the 'OK' button. Click the red slashed circle button to cancel.</font></center>
			"}