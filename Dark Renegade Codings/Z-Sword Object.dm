	...
	Sword
		...
		Click()
			if(src in usr:contents)
				...
				if(usr.level<1000)
					usr<<"<font color=blue>You need to be level 1000 or higher to be able to pick up this sword."
					return
				else
					if(!usr.swordequiped)
						if(usr.back=="EMPTY")
		...
		Click()
			if(src in usr:contents)
				...
				if(usr.level<500)
					...
					if(!usr.trunkswordequiped)
						if(usr.back=="EMPTY")
							if(src.item_owner!=usr.key&&src.item_owner)
								usr<<"Error: This item is Bio-Linked to someone!"
								return
							usr.trunkswordequiped=1
							usr.trunks_saved_str=usr.strength_max
							usr.strength_max+=usr.strength_max*0.075
							usr.overlays += sword
							usr<<"You have equiped the: <u>[src]</u>."
							suffix = "{Worn}"
							src.equiped = 1
							usr.back = "Trunks Sword"
						else
							usr<<"Error: You already have something equipped on your shoulders!"
							return
					else
						usr.trunkswordequiped=0
						usr.strength_max-= usr.trunks_saved_str * 0.075
						usr.overlays -= sword
						usr<<"You have unequiped the: <u>[src]</u>."
						suffix = null
						src.equiped = 0
		...
		Click()
			...
			if(src in usr:contents)
				if(!usr.legendarysword)
					...
					usr<<"You have equiped the: <u>[src]</u>."
					suffix="{Worn}"
					usr.strength+=75000*usr.level/1.5
					...
					usr.LSwordDrain(usr.powerlevel/2)
				else
					usr.overlays-=src.icon
					usr<<"You have unequiped the: <u>[src]</u>."
					usr.speed=usr.speed_max
					for(var/obj/Technique/SwordSlash/S in usr.contents)del(S)
					for(var/obj/Technique/ThousandSwordSlash/S in usr.contents)del(S)
					suffix=null
		...
		LSwordDrain(drain)
			if(src.legendarysword)
				...
				if(src.powerlevel<1)
					...
					for(var/obj/Technique/SwordSlash/S in src.contents)del(S)
					for(var/obj/Equipment/LegendarySword/L in src.contents)L.suffix=null
					src<<"You are too weak to wield your sword."
					src.speed=usr.speed_max
					return
				else spawn(15)if(src)src.LSwordDrain(drain)