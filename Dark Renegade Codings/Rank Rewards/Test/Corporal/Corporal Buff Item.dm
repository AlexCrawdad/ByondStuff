		...
		Click()
			if(src in usr:contents)
				...
				if("str_buff" in usr.buffs)
					...
					if(usr.strshardusing&&usr.strbuffusing<1)
						usr.BUFF_TIMER()
						usr.strbuffusing++
					return
				if(usr.active_buffs >= 3)
					...
					return
				for(var/obj/Equipment/Buff_Items/STR_Shard/A in usr)if(A.owner==usr.name&&A.cooldown>0)
					usr<<"You already have [A] in their cool down period!"
					return
				for(var/obj/Equipment/Buff_Items/STR_Shard/A in world)if(A.owner==usr.name&&A.cooldown>0)
					usr<<"You already have [A] in their cool down period!"
					return
				else
